...
def MILO (Input, TissueType, 
    ...
    if NoiseCorrection:
        if sum(df['Prob(MSI)'] > Prob_cutoff) == 0:
            ...
            df_tmp = pd.DataFrame({'sample' : pred_MSI_samples,
                                    '1_Del_T' : [0] * len(pred_MSI_samples),
                                    '2_Del_R' : [0] * len(pred_MSI_samples),
                                    '3_Del_R' : [0] * len(pred_MSI_samples), 
                                    '4_Del_R' : [0] * len(pred_MSI_samples),
                                    '5_Del_R' : [0] * len(pred_MSI_samples)})
                
            for s in pred_MSI_samples:
                for i in items:
                    value = noise_corrected_MSI_norm.loc[s, 
                                [i in c for c in noise_corrected_MSI_norm.columns]].sum()
                    df_tmp.loc[df_tmp['sample']== s, i] = value
